...
# --- the recorded run ------------------------------------------------------------------------
#
# The verdict table used to be published as console text in eval-output.md and compared to
# eval.json row by row. It is not published as prose any more: `docs/data/eval.json` is the
# result, and the same rule the README follows for the test count applies here — the tool states
# its own totals, and a document that restates them is a second copy that can drift. It did drift
# once, when the table was regenerated by hand after Q19 and Q20 were added.
#
# What is left is the pair of checks that actually carry weight. The bare-clone tests hold the
# record to what the spec declares, with no credentials. The cluster-gated test re-runs the eval
# and asserts the record still reproduces, which is what catches a result that stopped being true.
...
def test_every_case_in_the_set_is_in_the_record(case_id: str) -> None:
    """A case added to the set and never re-published would be silently unmeasured."""
    recorded = [c for c in EVAL_DATA["cases"] if c["id"] == case_id]
    assert len(recorded) == 1, f"{case_id} is not in docs/data/eval.json — re-run publish_eval.py"
    assert recorded[0]["verdict"] in {"PASS", "KNOWN"}


def test_the_recorded_totals_add_up_to_the_set() -> None:
    totals = EVAL_DATA["totals"]
    assert totals["fail"] == 0
    assert totals["pass"] + totals["known"] == len(CASES)
    assert totals["known"] == len(DECLARED_DEVIATIONS), (
        "the record carries a different number of known deviations than docs/specs/eval.md "
        "declares — one of the two moved without the other"
    )
...
def test_the_recorded_run_still_reproduces(bundle) -> None:
    ...
    assert not moved, (
        ...
        f"re-run scripts/publish_eval.py and read the diff before committing it"
    )
